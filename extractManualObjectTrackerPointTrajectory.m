function [t1, x1, y1, t2, x2, y2] = extractManualObjectTrackerPointTrajectory(trackingFilePath, videoFrame, plotTrajectory)
...
%    plotTrajectory is an optional boolean flag indicating whether or not 
%       to plot the extracted trajectories. Default is false.
%
...
if ~exist('videoFrame', 'var') || isempty('videoFrame')
    videoFrame = [];
end
if ~exist('plotTrajectory', 'var') || isempty('plotTrajectory')
...
%   value, an error will be raised.
    ...
    elseif length(coordList) == 1
        ...
        coordinate = coordList(1);
        if length(unique(coordList)) > 1
            warning('This function requires only one point per trajectory per frame, but multiple were found. Using the first one.');
        end
    end
end